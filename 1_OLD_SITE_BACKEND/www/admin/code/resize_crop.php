...
function resize($file_input, $file_output, $w_o, $h_o, $percent = false) {
	...
	if (!$w_i || !$h_i) {
		echo 'Resize -Невозможно получить длину и ширину изображения<br>';
		return;
    }
    $types = array('','gif','jpeg','png');
    $ext = $types[$type];
	  if ($ext) {    	$func = 'imagecreatefrom'.$ext;
    	$img = $func($file_input);    } else {    	echo 'Некорректный формат файла<br>';
		return;    }
...
function crop($file_input, $file_output, $crop = 'square', $percent = false) {
	list($w_i, $h_i, $type) = getimagesize($file_input);
	if (!$w_i || !$h_i) {
		echo 'Crop - Невозможно получить длину и ширину изображения<br>';
		return;
    }
    $types = array('','gif','jpeg','png');
    $ext = $types[$type];

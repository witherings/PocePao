...
if(isset($_POST['edit'])){
	...
	if ($uploadOk == 0) {
	  ...
	  if (move_uploaded_file($_FILES["img"]["tmp_name"], $new_file)) {
			mysql_query("UPDATE slider SET img='$file_name' WHERE id='$id'", $db);
...
if(isset($_POST['add'])){
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["img"]["tmp_name"], $new_file)) {
				mysql_query("INSERT INTO slider (img, title, text) VALUES ('$file_name','$title','$text')", $db);
...
if(isset($action)){include "template/content/articles/$action.php";}else{?>
...
?>
                                <tr id="<?php echo $data['id']?>">
                                    ...
                                    <td class="issue-info"><img src="../assets/images/slider/<?php echo $data['img']?>" style="height:50px;" alt=""> <?php echo $data['text'];?></td>
                                    <td class="text-right">
																			<a href="index.php?item=<?php echo $item?>&action=edit&id=<?php echo $data['id'];?>" class="btn btn-info"> <i class="fa fa-pencil"></i></a>
                                         <button type="button" class="btn btn-danger" onClick="delete_item('slider','<?php echo $data['id']?>','<?php echo $data['id']?>');"> <i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>

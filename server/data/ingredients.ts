import type { InsertIngredient } from "@shared/schema";

export function createIngredients(): InsertIngredient[] {
  return [
    // PROTEINS (with size-based pricing: Klein vs Standard)
    {
      name: "Tofu",
      nameDE: "Tofu",
      type: "protein",
      description: "Marinated tofu",
      descriptionDE: "Marinierter Tofu",
      image: "/images/wunschbowl/tofu cubes.png",
      priceSmall: "9.50",      // Klein bowl price
      priceStandard: "14.75",  // Standard bowl price
      order: 1,
      available: 1,
    },
    {
      name: "Falafel",
      nameDE: "Falafel",
      type: "protein",
      description: "Homemade falafel",
      descriptionDE: "Hausgemachte Falafel",
      image: "/images/wunschbowl/falafel balls.png",
      priceSmall: "9.50",
      priceStandard: "14.75",
      order: 2,
      available: 1,
    },
    {
      name: "Chicken",
      nameDE: "Hähnchen",
      type: "protein",
      description: "Tender chicken",
      descriptionDE: "Zartes Hähnchen",
      image: "/images/wunschbowl/chicken slices.png",
      priceSmall: "9.50",
      priceStandard: "14.75",
      order: 3,
      available: 1,
    },
    {
      name: "Salmon",
      nameDE: "Lachs",
      type: "protein",
      description: "Fresh salmon",
      descriptionDE: "Frischer Lachs",
      image: "/images/wunschbowl/salmon slices .png",
      priceSmall: "9.90",
      priceStandard: "15.90",
      order: 4,
      available: 1,
    },
    {
      name: "Shrimp",
      nameDE: "Garnelen",
      type: "protein",
      description: "Fresh shrimp",
      descriptionDE: "Frische Garnelen",
      image: "/images/wunschbowl/shrimp.png",
      priceSmall: "9.90",
      priceStandard: "15.90",
      order: 5,
      available: 1,
    },
    {
      name: "Tuna",
      nameDE: "Thunfisch",
      type: "protein",
      description: "Fresh tuna",
      descriptionDE: "Frischer Thunfisch",
      image: "/images/wunschbowl/tuna slices.png",
      priceSmall: "9.90",
      priceStandard: "15.90",
      order: 6,
      available: 1,
    },

    // BASES
    {
      name: "Rice",
      nameDE: "Reis",
      type: "base",
      description: "White rice",
      descriptionDE: "Weißer Reis",
      image: "/images/wunschbowl/white rice.png",
      order: 1,
      available: 1,
    },
    {
      name: "Couscous",
      nameDE: "Couscous",
      type: "base",
      description: "Couscous",
      descriptionDE: "Couscous",
      image: "/images/wunschbowl/cooked couscous.png",
      order: 2,
      available: 1,
    },
    {
      name: "Quinoa",
      nameDE: "Quinoa",
      type: "base",
      description: "Quinoa",
      descriptionDE: "Quinoa",
      image: "/images/wunschbowl/cooked quinoa.png",
      order: 3,
      available: 1,
    },
    {
      name: "Zucchini Noodles",
      nameDE: "Zucchini-Nudeln",
      type: "base",
      description: "Spiralized zucchini noodles",
      descriptionDE: "Zucchini-Nudeln",
      image: "/images/wunschbowl/zucchini spaghetti.png",
      order: 4,
      available: 1,
    },

    // MARINADES
    {
      name: "Lanai",
      nameDE: "Lanai",
      type: "marinade",
      description: "With soy and garlic",
      descriptionDE: "mit Soja, Knoblauch",
      image: "/images/wunschbowl/Lanai (Marinade).png",
      order: 1,
      available: 1,
    },
    {
      name: "Gozo",
      nameDE: "Gozo",
      type: "marinade",
      description: "With garlic",
      descriptionDE: "mit Knoblauch",
      image: "/images/wunschbowl/Gozo (Marinade).png",
      order: 2,
      available: 1,
    },
    {
      name: "Capri",
      nameDE: "Capri",
      type: "marinade",
      description: "Tomato-based, medium spicy",
      descriptionDE: "Ein Traum aus Tomaten",
      image: "/images/wunschbowl/Capri (Marinade).png",
      order: 3,
      available: 1,
    },
    {
      name: "Maui",
      nameDE: "Maui",
      type: "marinade",
      description: "Careful, fiery hot",
      descriptionDE: "Vorsicht, feurig-scharf",
      image: "/images/wunschbowl/Maui (Marinade).png",
      order: 4,
      available: 1,
    },

    // FRESH INGREDIENTS
    {
      name: "Edamame",
      nameDE: "Edamame",
      type: "fresh",
      description: "Steamed edamame beans",
      descriptionDE: "Edamame",
      image: "/images/wunschbowl/Edamame.png",
      order: 1,
      available: 1,
    },
    {
      name: "Cucumber",
      nameDE: "Gurke",
      type: "fresh",
      description: "Fresh cucumber",
      descriptionDE: "Gurke",
      image: "/images/wunschbowl/Gurke (Cucumber).png",
      order: 2,
      available: 1,
    },
    {
      name: "Avocado",
      nameDE: "Avocado",
      type: "fresh",
      description: "Fresh avocado",
      descriptionDE: "Avocado",
      image: "/images/wunschbowl/avokado.png",
      order: 3,
      available: 1,
    },
    {
      name: "Mango",
      nameDE: "Mango",
      type: "fresh",
      description: "Fresh mango cubes",
      descriptionDE: "Mango",
      image: "/images/wunschbowl/diced mango.png",
      order: 4,
      available: 1,
    },
    {
      name: "Carrot",
      nameDE: "Karotte",
      type: "fresh",
      description: "Shredded carrot",
      descriptionDE: "Karotte",
      image: "/images/wunschbowl/marinated carrot.png",
      order: 5,
      available: 1,
    },
    {
      name: "Kimchi",
      nameDE: "Kimchi",
      type: "fresh",
      description: "Korean fermented kimchi",
      descriptionDE: "Kimchi",
      image: "/images/wunschbowl/Haus-Kimchi (Top).png",
      order: 6,
      available: 1,
    },
    {
      name: "Cherry Tomatoes",
      nameDE: "Kirschtomaten",
      type: "fresh",
      description: "Fresh cherry tomatoes",
      descriptionDE: "Kirschtomaten",
      image: "/images/wunschbowl/fresh tomato cubes.png",
      order: 7,
      available: 1,
    },
    {
      name: "Spinach",
      nameDE: "Spinat",
      type: "fresh",
      description: "Fresh spinach leaves",
      descriptionDE: "Spinat",
      image: "/images/wunschbowl/fresh spinach leaves.png",
      order: 8,
      available: 1,
    },
    {
      name: "Sweet Potato",
      nameDE: "Süßkartoffel",
      type: "fresh",
      description: "Roasted sweet potato cubes",
      descriptionDE: "Süßkartoffel",
      image: "/images/wunschbowl/cubed sweet potato.png",
      order: 9,
      available: 1,
    },
    {
      name: "Pumpkin",
      nameDE: "süßer Kürbis",
      type: "fresh",
      description: "Roasted pumpkin cubes",
      descriptionDE: "süßer Kürbis",
      image: "/images/wunschbowl/sweet pumpkin.png",
      order: 10,
      available: 1,
    },
    {
      name: "Corn",
      nameDE: "Zuckermais",
      type: "fresh",
      description: "Sweet corn kernels",
      descriptionDE: "Zuckermais",
      image: "/images/wunschbowl/sweet corn kernels.png",
      order: 12,
      available: 1,
    },
    {
      name: "Red Onion",
      nameDE: "Rote Zwiebeln",
      type: "fresh",
      description: "Sliced red onion",
      descriptionDE: "Rote Zwiebeln",
      image: "/images/wunschbowl/marinated red onions.png",
      order: 13,
      available: 1,
    },
    {
      name: "Red Beet",
      nameDE: "Rote Bete",
      type: "fresh",
      description: "Pickled red beet",
      descriptionDE: "Rote Bete",
      image: "/images/wunschbowl/sliced beetroot.png",
      order: 14,
      available: 1,
    },
    {
      name: "Wakame Salad",
      nameDE: "Wakame Salat",
      type: "fresh",
      description: "Seaweed salad",
      descriptionDE: "Wakame Salat",
      image: "/images/wunschbowl/Wakame seaweed salad.png",
      order: 15,
      available: 1,
    },
    {
      name: "Pickled Radish",
      nameDE: "Rettich eingelegt",
      type: "fresh",
      description: "Pickled yellow radish",
      descriptionDE: "Rettich eingelegt",
      image: "/images/wunschbowl/pickled radish.png",
      order: 16,
      available: 1,
    },
    {
      name: "Pickled Ginger",
      nameDE: "Ingwer eingelegt",
      type: "fresh",
      description: "Pickled ginger slices",
      descriptionDE: "Ingwer eingelegt",
      image: "/images/wunschbowl/pickled white.png",
      order: 17,
      available: 1,
    },

    // SAUCES
    {
      name: "Teriyaki Sauce",
      nameDE: "Teriyaki-Sauce",
      type: "sauce",
      description: "Fresh arugula with creamy herbs",
      descriptionDE: "Frischer Rucola mit cremigen Kräutern",
      image: "/images/wunschbowl/Teriyaki Sauce .png",
      order: 1,
      available: 1,
    },
    {
      name: "Truffle Fusion",
      nameDE: "Trüffel Fusion",
      type: "sauce",
      description: "Noble truffle with tropical note",
      descriptionDE: "Edler Trüffel mit tropischer Note",
      image: "/images/wunschbowl/Trüffel Fusion Sauce.png",
      order: 2,
      available: 1,
    },
    {
      name: "Mango Dill",
      nameDE: "Mango-Dill",
      type: "sauce",
      description: "Sweet mango meets fresh dill",
      descriptionDE: "Süße Mango trifft frischen Dill",
      image: "/images/wunschbowl/Mango-Dill Sauce.png",
      order: 3,
      available: 1,
    },
    {
      name: "Chili Mayo",
      nameDE: "Chili Mayo",
      type: "sauce",
      description: "Creamy and pleasantly spicy",
      descriptionDE: "Cremig und angenehm scharf",
      image: "/images/wunschbowl/Chili Mayo.png",
      order: 4,
      available: 1,
    },
    {
      name: "Kimchi Sriracha",
      nameDE: "Kimchi Sriracha",
      type: "sauce",
      description: "Spicy and tangy - full of flavor",
      descriptionDE: "Würzig u scharf – voll im Geschmack",
      image: "/images/wunschbowl/Kimchi Sriracha.png",
      order: 5,
      available: 1,
    },
    {
      name: "Soy-Sesame",
      nameDE: "Soja-Sesam",
      type: "sauce",
      description: "Nutty sesame with fine soy sauce",
      descriptionDE: "Nussiger Sesam mit feiner Sojasoße",
      image: "/images/wunschbowl/Soy-Sesame Sauce.png",
      order: 6,
      available: 1,
    },
    {
      name: "Ginger Sauce",
      nameDE: "Ingwer-Sauce",
      type: "sauce",
      description: "Ginger sauce",
      descriptionDE: "Ingwer-Sauce",
      image: "/images/wunschbowl/Ginger Sauce.png",
      order: 7,
      available: 1,
    },

    // TOPPINGS
    {
      name: "Sesame",
      nameDE: "Sesam",
      type: "topping",
      description: "White sesame seeds",
      descriptionDE: "Sesam",
      image: "/images/wunschbowl/sesame seeds.png",
      order: 1,
      available: 1,
    },
    {
      name: "Sunflower Seeds",
      nameDE: "Sonnenblumenkerne",
      type: "topping",
      description: "Roasted sunflower seeds",
      descriptionDE: "Sonnenblumenkerne",
      image: "/images/wunschbowl/sunflower seeds.png",
      order: 2,
      available: 1,
    },
    {
      name: "Nori",
      nameDE: "Nori",
      type: "topping",
      description: "Dried seaweed sheets",
      descriptionDE: "Nori",
      image: "/images/wunschbowl/Nori strips.png",
      order: 3,
      available: 1,
    },
    {
      name: "Pomegranate",
      nameDE: "Granatapfel",
      type: "topping",
      description: "Pomegranate seeds",
      descriptionDE: "Granatapfel",
      image: "/images/wunschbowl/pomegranate seeds.png",
      order: 4,
      available: 1,
    },
    {
      name: "Spring Onions",
      nameDE: "Frühlingszwiebe",
      type: "topping",
      description: "Chopped spring onions",
      descriptionDE: "Frühlingszwiebe",
      image: "/images/wunschbowl/spring onions.png",
      order: 5,
      available: 1,
    },
    {
      name: "Black Olives",
      nameDE: "Schwarze Oliven",
      type: "topping",
      description: "Sliced black olives",
      descriptionDE: "Schwarze Oliven",
      image: "/images/wunschbowl/black olives.png",
      order: 8,
      available: 1,
    },
    {
      name: "Pumpkin Seeds",
      nameDE: "Kürbiskerne",
      type: "topping",
      description: "Roasted pumpkin seeds",
      descriptionDE: "Kürbiskerne",
      image: "/images/wunschbowl/pumpkin seeds.png",
      order: 9,
      available: 1,
    },
    {
      name: "Cashew Nuts",
      nameDE: "Cashewkerne",
      type: "topping",
      description: "Roasted cashew nuts",
      descriptionDE: "Cashewkerne",
      image: "/images/wunschbowl/cashew nuts.png",
      order: 10,
      available: 1,
    },
    {
      name: "Peanuts",
      nameDE: "Erdnüsse",
      type: "topping",
      description: "Roasted peanuts",
      descriptionDE: "Erdnüsse",
      image: "/images/wunschbowl/peanuts .png",
      order: 11,
      available: 1,
    },
    {
      name: "Coconut Chips",
      nameDE: "Kokoschips",
      type: "topping",
      description: "Crispy coconut chips",
      descriptionDE: "Kokoschips",
      image: "/images/wunschbowl/coconut chips.png",
      order: 12,
      available: 1,
    },
    {
      name: "Roasted Beet Chips",
      nameDE: "Geröstete Rote Bete Chips",
      type: "topping",
      description: "Roasted red beet chips",
      descriptionDE: "Geröstete Rote Bete Chips",
      image: "/images/wunschbowl/oasted beetroot.png",
      order: 13,
      available: 1,
    },
    {
      name: "Banana Chips",
      nameDE: "Bananenchips",
      type: "topping",
      description: "Crispy banana chips",
      descriptionDE: "Bananenchips",
      image: "/images/wunschbowl/banana chips.png",
      order: 14,
      available: 1,
    },
    {
      name: "Capers",
      nameDE: "Kapern",
      type: "topping",
      description: "Pickled capers",
      descriptionDE: "Kapern",
      image: "/images/wunschbowl/capers.png",
      order: 15,
      available: 1,
    },
    {
      name: "Almonds",
      nameDE: "Mandeln",
      type: "topping",
      description: "Sliced almonds",
      descriptionDE: "Mandeln",
      image: "/images/wunschbowl/almonds.png",
      order: 16,
      available: 1,
    },
    {
      name: "Coconut Flakes",
      nameDE: "Kokosraspeln",
      type: "topping",
      description: "Shredded coconut",
      descriptionDE: "Kokosraspeln",
      image: "/images/wunschbowl/coconut chips.png",
      order: 17,
      available: 1,
    },
    {
      name: "Pickled Ginger",
      nameDE: "Ingwer eingelegt",
      type: "topping",
      description: "Pickled ginger slices",
      descriptionDE: "Ingwer eingelegt",
      image: "/images/wunschbowl/pickled white.png",
      order: 18,
      available: 1,
    },

    // EXTRA PROTEINS (with pricing)
    {
      name: "Extra Tofu",
      nameDE: "Extra Tofu",
      type: "extra_protein",
      description: "Additional marinated tofu",
      descriptionDE: "Zusätzlicher marinierter Tofu",
      image: "/images/wunschbowl/tofu cubes.png",
      price: "3.00",
      order: 1,
      available: 1,
    },
    {
      name: "Extra Falafel",
      nameDE: "Extra Falafel",
      type: "extra_protein",
      description: "Additional homemade falafel",
      descriptionDE: "Zusätzliche hausgemachte Falafel",
      image: "/images/wunschbowl/falafel balls.png",
      price: "3.00",
      order: 2,
      available: 1,
    },
    {
      name: "Extra Chicken",
      nameDE: "Extra Hähnchen",
      type: "extra_protein",
      description: "Additional tender chicken",
      descriptionDE: "Zusätzliches zartes Hähnchen",
      image: "/images/wunschbowl/chicken slices.png",
      price: "3.00",
      order: 3,
      available: 1,
    },
    {
      name: "Extra Salmon",
      nameDE: "Extra Lachs",
      type: "extra_protein",
      description: "Additional fresh salmon",
      descriptionDE: "Zusätzlicher frischer Lachs",
      image: "/images/wunschbowl/salmon slices .png",
      price: "3.50",
      order: 4,
      available: 1,
    },
    {
      name: "Extra Shrimp",
      nameDE: "Extra Garnelen",
      type: "extra_protein",
      description: "Additional fresh shrimp",
      descriptionDE: "Zusätzliche frische Garnelen",
      image: "/images/wunschbowl/shrimp.png",
      price: "3.50",
      order: 5,
      available: 1,
    },
    {
      name: "Extra Tuna",
      nameDE: "Extra Thunfisch",
      type: "extra_protein",
      description: "Additional fresh tuna",
      descriptionDE: "Zusätzlicher frischer Thunfisch",
      image: "/images/wunschbowl/tuna slices.png",
      price: "3.50",
      order: 6,
      available: 1,
    },

    // EXTRA FRESH INGREDIENTS (with pricing)
    {
      name: "Extra Edamame",
      nameDE: "Extra Edamame",
      type: "extra_fresh",
      description: "Additional edamame beans",
      descriptionDE: "Zusätzliche Edamame",
      image: "/images/wunschbowl/Edamame.png",
      price: "1.50",
      order: 1,
      available: 1,
    },
    {
      name: "Extra Avocado",
      nameDE: "Extra Avocado",
      type: "extra_fresh",
      description: "Additional fresh avocado",
      descriptionDE: "Zusätzliche frische Avocado",
      image: "/images/wunschbowl/avokado.png",
      price: "2.00",
      order: 2,
      available: 1,
    },
    {
      name: "Extra Mango",
      nameDE: "Extra Mango",
      type: "extra_fresh",
      description: "Additional fresh mango cubes",
      descriptionDE: "Zusätzliche frische Mango",
      image: "/images/wunschbowl/diced mango.png",
      price: "1.50",
      order: 3,
      available: 1,
    },
    {
      name: "Extra Kimchi",
      nameDE: "Extra Kimchi",
      type: "extra_fresh",
      description: "Additional Korean fermented kimchi",
      descriptionDE: "Zusätzliches Kimchi",
      image: "/images/wunschbowl/Haus-Kimchi (Top).png",
      price: "1.50",
      order: 4,
      available: 1,
    },

    // EXTRA SAUCES (with pricing)
    {
      name: "Extra Teriyaki Sauce",
      nameDE: "Extra Teriyaki-Sauce",
      type: "extra_sauce",
      description: "Additional teriyaki sauce",
      descriptionDE: "Zusätzliche Teriyaki-Sauce",
      image: "/images/wunschbowl/Teriyaki Sauce .png",
      price: "1.00",
      order: 1,
      available: 1,
    },
    {
      name: "Extra Truffle Fusion",
      nameDE: "Extra Trüffel Fusion",
      type: "extra_sauce",
      description: "Additional truffle fusion sauce",
      descriptionDE: "Zusätzliche Trüffel Fusion",
      image: "/images/wunschbowl/Trüffel Fusion Sauce.png",
      price: "1.50",
      order: 2,
      available: 1,
    },
    {
      name: "Extra Chili Mayo",
      nameDE: "Extra Chili Mayo",
      type: "extra_sauce",
      description: "Additional chili mayo",
      descriptionDE: "Zusätzliche Chili Mayo",
      image: "/images/wunschbowl/Chili Mayo.png",
      price: "1.00",
      order: 3,
      available: 1,
    },

    // EXTRA TOPPINGS (with pricing)
    {
      name: "Extra Sesame",
      nameDE: "Extra Sesam",
      type: "extra_topping",
      description: "Additional sesame seeds",
      descriptionDE: "Zusätzlicher Sesam",
      image: "/images/wunschbowl/sesame seeds.png",
      price: "0.50",
      order: 1,
      available: 1,
    },
    {
      name: "Extra Pomegranate",
      nameDE: "Extra Granatapfel",
      type: "extra_topping",
      description: "Additional pomegranate seeds",
      descriptionDE: "Zusätzlicher Granatapfel",
      image: "/images/wunschbowl/pomegranate seeds.png",
      price: "1.00",
      order: 2,
      available: 1,
    },
    {
      name: "Extra Cashew Nuts",
      nameDE: "Extra Cashewkerne",
      type: "extra_topping",
      description: "Additional roasted cashew nuts",
      descriptionDE: "Zusätzliche Cashewkerne",
      image: "/images/wunschbowl/cashew nuts.png",
      price: "1.00",
      order: 3,
      available: 1,
    },
  ];
}
